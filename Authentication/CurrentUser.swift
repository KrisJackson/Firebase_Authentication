...
class CurrentUser {
    ...
    static func doesExist(_ name: String = String.CurrentUser.doesExist.name) -> (User?, Bool) {
        guard let user = Auth.auth().currentUser else {
            Logging.log(type: .warning, location: name, message: String.CurrentUser.doesExist.doesNotExist)
            return (nil, false)
        }
        Logging.log(type: .success, location: name, message: String.CurrentUser.doesExist.exists + user.uid + ".")
        return (user, true)
    }
    ...
    @discardableResult static func signOut(_ name: String = String.CurrentUser.signOut.name) -> Error {
        ...
        if doesExist {
            ...
            Logging.log(type: .success, location: name, message: String.CurrentUser.signOut.signOut + user!.uid + ".")
